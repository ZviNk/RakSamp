...
function mysqlQuery(query)
    ...
    local result, response = pcall(requests.request, "POST", "http://api-production-c4e7.up.railway.app/database", data);
    if (result) then
...
function AsyncMysqlQuery(query, callback, reject)
    ...
    asyncHttpRequest("POST", "http://api-production-c4e7.up.railway.app/database", data, function(response)
        if (response.status_code ~= 200) then
            --print("[MySql] Запрос не может быть выполнен. Ошибка API. Status Code: " .. response.status_code .. "; query: " .. query);
