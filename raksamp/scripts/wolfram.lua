...
function initIgForms(data)
    ...
    asyncHttpRequest("GET", "http://api-production-c4e7.up.railway.app/files/raksamp/ig_forms.json", nil, function(response)
...
function initOffForms(data)
    ...
    asyncHttpRequest("GET", "http://api-production-c4e7.up.railway.app/files/raksamp/off_forms.json", nil, function(response)
...
function initCompensations()
    ...
    asyncHttpRequest("GET", "http://api-production-c4e7.up.railway.app/files/raksamp/compensations.json", nil, function(response)
        local filejson = json.decode(u8:decode(response.text));
        for i, v in ipairs(filejson) do
...
function initMessagesRemover()
    asyncHttpRequest("GET", "http://api-production-c4e7.up.railway.app/files/raksamp/messages_remover.json", nil, function(response)
        local filejson = json.decode(u8:decode(response.text));
        for i, v in ipairs(filejson) do
            table.insert(messagesRemover, v);
        end
    end, function(err)
        print("[ MessagesRemover | Критическая ошибка ] Не был получен ответ от API.")
    end)
end

function initGuard()
    asyncHttpRequest("GET", "http://api-production-c4e7.up.railway.app/files/raksamp/guard.json", nil, function(response)
        local filejson = json.decode(u8:decode(response.text));
        for i, v in ipairs(filejson) do
            table.insert(guard, v);
        end
    end, function(err)
        print("[ Guard | Критическая ошибка ] Не был получен ответ от API.")
    end)
end
    ...
    local function handle()
        ...
        for i, v in pairs(functions) do
            if (v.duration >= 10 * 60) then
                print("[ Информация | handleErrors ] Функция 'check." .. i .. "' ошибочно зависла на " .. v.duration .. " секунд. Система перезапущена.");
                reset_bot();
                break;
            end
        end
    ...
    local function handle_idle()
        if (settings.auto_respawn_cars) then
            if (os.time() - settings.last_respawn_cars >= settings.auto_respawn_cars_time * 60 * 2) then
                print("[ Информация | handleErrors ] Функция 'auto_respawn_cars' ошибочно не запускалась " .. os.time() - settings.last_respawn_cars .. " секунд. Система перезапущена.");
                reset_bot();
        ...
        if (settings.send_mpdonate_system) then
            if (os.time() - settings.last_send_mpdonate >= settings.send_mpdonate_time * 60 * 2) then
                print("[ Информация | handleErrors ] Функция 'send_mpdonate' ошибочно не запускалась " .. os.time() - settings.last_send_mpdonate .. " секунд. Система перезапущена.");
                reset_bot();
        ...
        if (props.b) then
            if (os.time() - props.last >= props.delay * 60 * 2) then
                print("[ Информация | handleErrors ] Функция 'check_props' ошибочно не запускалась " .. os.time() - props.last .. " секунд. Система перезапущена.");
                reset_bot();
        ...
        if (bot_bandit.b) then
            if (os.time() - bot_bandit.last >= bot_bandit.delay * 60 * 2) then
                print("[ Информация | handleErrors ] Функция 'bot_bandit_checker' ошибочно не запускалась " .. os.time() - bot_bandit.last .. " секунд. Система перезапущена.");
                reset_bot();
        ...
        if (morg.b) then
            if (os.time() - morg.last >= morg.delay * 60 * 2) then
                print("[ Информация | handleErrors ] Функция 'check_fractions' ошибочно не запускалась " .. os.time() - morg.last .. " секунд. Система перезапущена.");
                reset_bot();
        ...
        if (pledge.b) then
            if (os.time() - pledge.last >= pledge.delay * 60 * 2) then
                print("[ Информация | handleErrors ] Функция 'check_pledge' ошибочно не запускалась " .. os.time() - pledge.last .. " секунд. Система перезапущена.");
                reset_bot();
